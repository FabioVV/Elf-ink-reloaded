...
func OpenDB() (*sql.DB, error) {
	...
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS task_item (id INTEGER PRIMARY KEY, name VARCHAR(100), content TEXT DEFAULT '', status SMALLINT DEFAULT 1, editing SMALLINT DEFAULT 0, task_id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (task_id) REFERENCES task(id) ON DELETE CASCADE)")
	if err != nil {
		db.Close()
